                              ...
                              struct slirp4netns_config *cfg)
{
    ...
    sai.sin6_port = 0;
    sai.sin6_addr = ifr6.ifr6_addr = cfg->recommended_vguest6;
...
#undef NB_BOOTP_CLIENTS
    ...
    if (cfg->enable_ipv6) {
        printf("* IPv6 Network:         %s\n",
               inet_ntop(AF_INET6, &cfg->vnetwork6, str, sizeof(str)));
        printf("* IPv6 Netmask:         %s\n",
               inet_ntop(AF_INET6, &cfg->vnetmask6, str, sizeof(str)));
        printf("* IPv6 Gateway:         %s\n",
               inet_ntop(AF_INET6, &cfg->vhost6, str, sizeof(str)));
        printf("* IPv6 DNS:             %s\n",
               inet_ntop(AF_INET6, &cfg->vnameserver6, str, sizeof(str)));
        printf(
            "* IPv6 Recommended IP:  %s\n",
            inet_ntop(AF_INET6, &cfg->recommended_vguest6, str, sizeof(str)));
    }
    if (api_socket != NULL) {
        printf("* API Socket:      %s\n", api_socket);
    }
